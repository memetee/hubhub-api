...
class UserController {
  ...
  async visitLog(ctx: Context, next: Next) {
    try {
      const { id } = ctx.user;
      const result1 = await userService.getVisitLog(id) as [Number];
      let countObj = result1[0] as any;
      let result2 = null;
      if (countObj && countObj.count) {
        countObj.count++
        result2 = await userService.updateVisitLog(id, countObj.count);
      } else {
        result2 = await userService.setVisitLog(id);
      }
      ctx.app.emit('response', types.VISIT_LOG, ctx)
    } catch(err) {
      console.log(err)
    }
  }

  async delVisitLog(ctx: Context, next: Next) {
    await userService.delVisitLog();
    ctx.app.emit('response', types.DEL_VISIT_LOG, ctx)
  }
}
